...
if __name__ == '__main__':
    ...
    p("vinserti128 $1, %xmm{}, %ymm{}, %ymm{}".format(y, t1, y_loxhi_lo))
    ...
    for j in range(1 + (SOFIA_N % n_accums > 0)):  # Separate 'regular' and 'final' accumulation rounds.
        ...
        p("vmovdqa {}(%r10), %ymm{}".format(32, xixj))
        p("add $32, %r10")
        p("vpermq ${}, %ymm{}, %ymm{}".format(int('01001110', 2), xixj, xixj_swapped))
        for k, acc in enumerate(accums):
            p("vpand {}(%rcx), %ymm{}, %ymm{}".format(32 * 66 * k, xixj, Fval))
            p("vpxor %ymm{}, %ymm{}, %ymm{}".format(Fval, acc[0], acc[0]))
        for k, acc in enumerate(accums):
            p("vpand {}(%rcx), %ymm{}, %ymm{}".format(32 * 66 * k, xixj_swapped, Fval))
        ...
        p("add ${}, %rcx".format((len(accums) * 66 - 66) * 32))
