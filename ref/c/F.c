...
{
#if SOFIA_N != 128 && SOFIA_M != 128
    #error "Dividing F in blocks depends on N = M = 128"
#endif
    unsigned char i;
    for (i = 0; i < 4; i++) {
        cshake128_simple((unsigned char *)F + i * (SOFIA_FBYTES >> 2), SOFIA_FBYTES >> 2, &i, 1, seed, SOFIA_SEEDBYTES);
    }
}
